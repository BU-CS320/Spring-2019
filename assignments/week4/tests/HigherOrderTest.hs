...
zipTest3 = QC.testProperty "For all Lists, zipWith should act like Prelude's zipWith" $ 
    \la lb -> (Prelude.zipWith (mod) (la::[Integer]) (lb::[Integer])) == (HigherOrderProblems.zipWith (mod) (la::[Integer]) (lb::[Integer]))
...
listAdderTests = testGroup "Tests for listAdder func" [
    ...
    testCase "listAdder should work" $ assertEqual [] ([3,5,7]) (listAdder [1,5,8,6,3] [2,9,8])
    ]
...
petTests = testGroup "Tests for Pet datatype" [
    ...
    QC.testProperty "For all Cats, they are conditionally happy" $
